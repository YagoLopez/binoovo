...
describe('Test pagination', () => {
  ...
  it('Should return 7 pages for ' + searchTerm1, () => {
    cy.visit('http://localhost:3000/movies-search/'+ searchTerm1 +'?page=1')
  ...
  it('Should be disabled next button for last page', () => {
    cy.visit('http://localhost:3000/movies-search/rambo?page=2')
    cy.get('[data-cy=next-btn]').should('be.disabled')
  })

  it('Should return no results for page out of range and not break', () => {
    cy.visit('http://localhost:3000/movies-search/'+ searchTerm1 +'?page=100')
    cy.get('[data-cy=no-results-msg]')
  })
